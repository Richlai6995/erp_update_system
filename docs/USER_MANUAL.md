...
6.  [SQLPlus 連線功能](#6-sqlplus-連線功能-sqlplus-terminal-access)
7.  [狀態追蹤、匯出與通知](#7-狀態追蹤匯出與通知)
8.  [注意事項](#8-注意事項)
...
## 6. SQLPlus 連線功能 (SQLPlus Terminal Access)

針對 **Terminal Access** 類型的申請單，且狀態為 **Approved (已審核)** 或 **Online (已上線)** 時，系統提供網頁版 SQLPlus 連線功能。

### 6.1 啟動連線
1.  進入申請單明細頁面。
2.  在「Terminal Connection History (SQLPlus 連線紀錄)」區塊標題旁，點擊藍色的 **「連線 (Connect SQLPlus)」** 按鈕。
3.  系統將彈出終端機視窗並自動登入至目標資料庫 (無需輸入密碼)。

### 6.2 連線紀錄
在此區塊下方會列出您的歷史連線紀錄，包含：
*   **使用者**：連線帳號。
*   **時間**：開始與結束時間。
*   **檢視紀錄 (View Log)**：點擊可查看該次連線的操作歷程 (為了安全起見，密碼相關指令會被自動遮蔽)。
